...
@section('content')
    ...
    <section class="m-3 lg:m-10">
        ...
        <div
            class="flex flex-col shadow-md border rounded-lg p-6 w-full h-full border-gray-200 dark:border-gray-700 bg-base-100 dark:bg-base-300">
                    ...
                    <div class="inline-block prose">
                        <code class="text-neutral-content bg-[color:var(--tw-prose-pre-bg)] rounded-sm">
                            @forelse ($maintenance->operators as $operator)
                                {{ $operator->username }}@if (!$loop->last)
                                    ,
                                @endif
                            @empty
                    ...
                    <div>
                        @forelse ($maintenance->companies as $company)
                            <a class="link link-hover link-primary tooltip tooltip-right"
                               href="{{ $company->show_url }}" data-tip="Voir la fiche Entreprise"><span
                                    class="font-bold">{{ $company->name }}</span></a>@if (!$loop->last)
                                ,
                            @endif
                        @empty
