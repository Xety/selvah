...
@section('content')
    ...
    <section class="m-3 lg:m-10">
                                ...
                                <div class="text-right">
                                    @if (
                                        ...
                                        <div class="dropdown dropdown-end">
                                            ...
                                            <ul tabindex="0"
                                                class="dropdown-content menu items-start p-2 shadow bg-base-100 rounded-box w-52 z-[1]">
                                                @can('update', \Selvah\Models\Part::class)
                                                    <li class="w-full">
                                                        <a href="{{ route('parts.index', ['editid' => $part->getKey(), 'edit' => 'true']) }}"
                                                           class="text-blue-500 tooltip tooltip-top text-left"
                                                           data-tip="Modifier cette pièce détachée">
                                                            <i class="fa-solid fa-pen-to-square"></i> Modifier cette
                                                            pièce
                                                        </a>
                                                    </li>
                                                @endcan
                                                @can('generateQrCode', \Selvah\Models\Part::class)
                                                    <li class="w-full">
                                                        <a href="{{ route('parts.index', ['qrcodeid' => $part->getKey(), 'qrcode' => 'true']) }}"
                                                           class="text-purple-500 tooltip tooltip-top text-left"
                                                           data-tip="Générer un QR Code pour cette pièce détachée">
                                                ...
                                                @can('create', \Selvah\Models\PartEntry::class)
                                                    <li class="w-full">
                                                        <a href="{{ route('part-entries.index', ['qrcodeid' => $part->getKey(), 'qrcode' => 'true']) }}"
                                                           class="text-green-500 tooltip tooltip-top text-left"
                                                           data-tip="Créer une entrée pour cette pièce détachée">
                                                            <i class="fa-solid fa-arrow-right-to-bracket"></i> Créer une
                                                            Entrée
                                                        </a>
                                                    </li>
                                                @endcan
                                                @can('create', \Selvah\Models\PartExit::class)
                                                    <li class="w-full">
                                                        <a href="{{ route('part-exits.index', ['qrcodeid' => $part->getKey(), 'qrcode' => 'true']) }}"
                                                           class="text-yellow-500 tooltip tooltip-top text-left"
                                                           data-tip="Créer une sortie pour cette pièce détachée">
                                                            <i class="fa-solid fa-right-from-bracket"></i> Créer une
                                                            Sortie
                                                        </a>
                                                    </li>
                                                @endcan
        ...
        <div class="grid grid-cols-12 gap-6 mb-7">
            <div
                class="col-span-12 border rounded-lg p-3 border-gray-200 dark:border-gray-700 bg-base-100 dark:bg-base-300">
                            ...
                            <x-slot name="body">
                                @forelse($partEntries as $partEntry)
                                    <x-table.row wire:loading.class.delay="opacity-50"
                                                 wire:key="row-{{ $partEntry->getKey() }}">
                                        <x-table.cell>{{ $partEntry->getKey() }}</x-table.cell>
                                        <x-table.cell>
                                            <a class="link link-hover link-primary font-bold"
                                               href="{{ $partEntry->part->show_url }}">
                                                {{ $partEntry->part->name }}
                                            </a>
                                        </x-table.cell>
                                        ...
                                        <x-table.cell>
                                            @unless (is_null($partExit->maintenance))
                                                <a class="link link-hover link-primary tooltip tooltip-right"
                                                   href="{{ $partExit->maintenance->show_url }}"
                                                   data-tip="Voir la fiche Maintenance">
                                                    <span
                                                        class="font-bold">{{ $partExit->maintenance->getKey() }}</span>
                                                </a>
                                            @endunless
                                        </x-table.cell>
                                        <x-table.cell>
                                            <a class="link link-hover link-primary font-bold"
                                               href="{{ $partExit->part->show_url }}">
                                                {{ $partExit->part->name }}
                                            </a>
                                        </x-table.cell>
